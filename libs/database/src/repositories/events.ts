...
} from "../schema/tables/event-settings";
...
class EventSelector {
  ...
  async getAvailableTeams(): Promise<Team[]> {
    ...
    const registeredTeamIds = new Set(
      registeredTeams.map((row) => row.team_id)
    );
  ...
  async registerTeams(registration: Record<string, string[]>): Promise<void> {
    return this.db.transaction().execute(async (trx) => {
      ...
      const rows = Object.entries(registration).flatMap(
        ([divisionId, _teamIds]) => {
          return _teamIds
  ...
  async updateSettings(
    updateData: UpdateableEventSettings
  ...
  async getAllSummaries(): Promise<EventSummary[]> {
    ...
    for (const row of result) {
      ...
      const divisionExists = event.divisions.some(
        (div: { id: string; name: string; color: string }) =>
          div.id === row.division_id
      );
      if (!divisionExists) {
...
export class EventsRepository {
  ...
  async create(event: InsertableEvent): Promise<Event> {
    ...
    await this.db
      .insertInto("event_settings")
      .values({ event_id: createdEvent.id })
      .execute();
