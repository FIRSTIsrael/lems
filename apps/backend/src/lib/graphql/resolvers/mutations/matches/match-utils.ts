 ...
 * @param currentMatch - ID of the current match to exclude from consideration
 * @returns The ID of the match to auto-load, or null if none is eligible
 */
export async function getAutoLoadMatch(
  divisionId: string,
  currentStage: string,
  currentMatch?: string
): Promise<string | null> {
  ...
  const divisionMatches = allMatches
    .filter(match => match.stage === currentStage && match.id !== currentMatch)
