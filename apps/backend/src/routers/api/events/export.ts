...
router.get('/rubrics/:judgingCategory', async (req: Request, res: Response) => {
  ...
  if (!schema) return res.status(400).json({ error: 'Invalid category' });
  ...
  return res.send(`\ufeff${parser.parse(csvData)}`);
});
