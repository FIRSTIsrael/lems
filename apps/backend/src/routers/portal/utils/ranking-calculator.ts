...
export async function calculateRobotGameRankings(
  ...
  // Fetch all submitted RANKING scoresheets for the division
  const scoresheets = await db.scoresheets.byDivision(divisionId).byStage('RANKING').getAll();

  // Filter to only submitted scoresheets with valid scores
  ...
  // Convert to array with metrics for sorting
  ...
  // Sort teams using compareScoreArrays (implements FLL ranking)
  // compareScoreArrays compares arrays element-by-element from highest to lowest
